...
class DashboardAPI(GenericAPIView):
  ...
  def get(self, request):
    ...
    } for rider in sorted(User.objects.filter(groups__name__in=['rider']), key=lambda a: sum([(order.ordered_subtotal-(order.ordered_commission if order.ordered_commission else 0))+order.ordered_shipping_commission for order in Order.objects.filter(rider=a, is_canceled=False, is_delivered=True)]) - sum([int(payment.amount) for payment in CommissionPayment.objects.filter(rider=a)]), reverse=True)]
